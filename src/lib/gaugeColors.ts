...
// `className`/`badgeClassName`/`textClassName` are full literal strings (not
// built via interpolation) since Tailwind's scanner needs to see the
// complete class name in source to generate it. `badgeClassName` is the
// outline style — border + tinted background + colored text reads better
// than white-on-color at these small badge sizes, especially against the
// dark gray "disabled" color where solid-fill white text had poor contrast.
// `textClassName` is just the text color alone, for coloring a bare value
// (e.g. a reading number) without the badge's border/background chrome.
export const GAUGE_COLORS = {
  cool: {
    ...
    textClassName: 'text-device-cool',
  },
  dry: {
