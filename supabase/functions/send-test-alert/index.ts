...
// real email over the same SMTP path climate-alerts uses, to the calling
// user's own address, so SMTP/Postmark setup can be verified without
// waiting on real telemetry or the 3-minute sustained-out-of-range window.
// See docs/climate-alerts.md.
//
...
import { createSmtpClient, sendEmail } from '../_shared/sendEmail.ts';
  ...
  try {
    await sendEmail(
      ...
      [
        ...
        'climate-alerts SMTP setup is working end to end.',
        '',
